...
const FilterGif = ({ alignLeft = false, showTrending = false }) => {
  ...
  return (
    ...
    >
      ...
      <div className="flex min-w-10 rounded-full bg-gray-800 justify-center">
        {filters.map((f) => (
          <span
            onClick={() => setFilter(f.value)}
            className={`${filter === f.value ? f.background : ""} font-semibold py-2 px-4 text-center rounded-full cursor-pointer`}
            key={f.title}
          >
            {f.title}
          </span>
